...
//!
//! Writes to `~/Library/Logs/Donny/donny.log` on macOS (the
...
//! [`MAX_LOG_BYTES`]; one rotated file (`donny.log.1`) is kept.
//!
...
const LOG_FILE_NAME: &str = "donny.log";
...
fn log_dir() -> PathBuf {
    ...
    {
        return base.home_dir().join("Library/Logs/Donny");
    }
    if let Some(dirs) = directories::ProjectDirs::from("", "", "Donny") {
        return dirs.data_local_dir().join("logs");
    }
    std::env::temp_dir().join("donny-logs")
}
