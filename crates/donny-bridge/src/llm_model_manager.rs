...
use donny_core::{AppSettings, LlmPreset};
use reqwest::blocking::Client;

const USER_AGENT: &str = "donny/0.1";
...
pub fn default_llm_model_path(preset: LlmPreset) -> Result<PathBuf, String> {
    let project_dirs = ProjectDirs::from("com", "getdonny", "Donny")
...
pub fn log_llm_inventory(settings: &AppSettings) {
    ...
    for entry in &settings.custom_llm_models {
        let location = match &entry.source {
            donny_core::CustomLlmSource::LocalPath { path } => {
            ...
            donny_core::CustomLlmSource::DownloadUrl { .. } => {
...
pub fn purge_legacy_llm_files() -> Result<Vec<String>, String> {
    use donny_core::LEGACY_LLM_FILENAMES;

    let Some(project_dirs) = ProjectDirs::from("com", "getdonny", "Donny") else {
        return Ok(Vec::new());
    };
