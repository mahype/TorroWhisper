...
use donny_core::{AppSettings, ModelPreset};
...
mod tests {
    ...
    fn write_temp_model(name: &str, contents: &[u8]) -> PathBuf {
        let path = std::env::temp_dir().join(format!("donny-test-{name}"));
    ...
    fn integrity_missing_for_absent_file() {
        let path = std::env::temp_dir().join("donny-test-does-not-exist.bin");
    ...
    fn cleanup_removes_only_part_files() {
        let dir = std::env::temp_dir().join("donny-test-partials");
