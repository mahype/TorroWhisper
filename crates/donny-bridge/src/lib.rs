...
impl BridgeRuntime {
    ...
    fn hermes_key_status(&self) -> Vec<donny_core::HermesKeyStatusDto> {
        self.settings
            .hermes_agents
            .iter()
            .map(|agent| donny_core::HermesKeyStatusDto {
                id: agent.id.clone(),
...
struct ApiKeySetRequest {
    backend: donny_core::LlmBackendKind,
    key: String,
}
...
pub extern "C" fn ow_get_llm_api_key_status() -> *mut c_char {
    use donny_core::{ApiKeyStatusDto, LlmBackendKind};
...
pub extern "C" fn ow_get_plugin_catalog() -> *mut c_char {
    response_ok(donny_core::builtin_plugin_catalog())
}
...
pub extern "C" fn ow_tts_piper_voices() -> *mut c_char {
    let voices: Vec<String> = donny_core::PIPER_VOICE_IDS
        .iter()
        .map(|id| (*id).to_owned())
        .collect();
    response_ok(voices)
...
mod tests {
    use super::*;
    use donny_core::DiagnosticStatus;
