...
//!
//! Writes to `~/Library/Logs/DonnyWhisper/donnywhisper.log` on macOS (the
...
//! [`MAX_LOG_BYTES`]; one rotated file (`donnywhisper.log.1`) is kept.
//!
...
const LOG_FILE_NAME: &str = "donnywhisper.log";
...
fn log_dir() -> PathBuf {
    ...
    {
        return base.home_dir().join("Library/Logs/DonnyWhisper");
    }
    if let Some(dirs) = directories::ProjectDirs::from("", "", "DonnyWhisper") {
        return dirs.data_local_dir().join("logs");
    }
    std::env::temp_dir().join("donnywhisper-logs")
}
...
impl Log for FileLogger {
    fn enabled(&self, metadata: &Metadata) -> bool {
        ...
        target.starts_with("donnywhisper")
            || matches!(
                target,
