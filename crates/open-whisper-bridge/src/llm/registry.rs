...
fn build_inner(
    ...
    // One final pass stamps the literal app-wide enable state onto every entry.
    // Literal membership (not the "empty = show all" fallback) so the management
    // UI shows true toggle state; the show-all fallback lives in the pickers.
