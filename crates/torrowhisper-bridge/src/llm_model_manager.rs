...
use torrowhisper_core::{AppSettings, LlmPreset};
...
pub fn log_llm_inventory(settings: &AppSettings) {
    ...
    for entry in &settings.custom_llm_models {
        let location = match &entry.source {
            torrowhisper_core::CustomLlmSource::LocalPath { path } => {
            ...
            torrowhisper_core::CustomLlmSource::DownloadUrl { .. } => {
...
pub fn purge_legacy_llm_files() -> Result<Vec<String>, String> {
    use torrowhisper_core::LEGACY_LLM_FILENAMES;

    let Some(project_dirs) = ProjectDirs::from("com", "gettorro", "TorroWhisper") else {
        return Ok(Vec::new());
    };
