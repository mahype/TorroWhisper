...
//!
//! Writes to `~/Library/Logs/TorroWhisper/torrowhisper.log` on macOS (the
...
//! [`MAX_LOG_BYTES`]; one rotated file (`torrowhisper.log.1`) is kept.
//!
...
const LOG_FILE_NAME: &str = "torrowhisper.log";
...
pub(crate) fn log_dir() -> PathBuf {
    ...
    {
        return base.home_dir().join("Library/Logs/TorroWhisper");
    }
    if let Some(dirs) = directories::ProjectDirs::from("", "", "TorroWhisper") {
        return dirs.data_local_dir().join("logs");
    }
    std::env::temp_dir().join("torrowhisper-logs")
}
