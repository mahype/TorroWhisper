...
1. `cargo build -p donny-bridge` — produces `target/debug/libdonny_bridge.a`
2. `swift run --package-path apps/donny-macos Donny` — launches the app

The Swift package links against `target/debug/libdonny_bridge.a` via the hard-coded linker flag in [Package.swift](../apps/donny-macos/Package.swift). Release builds go through the build script below instead.
...
This produces a **universal** (Apple Silicon + Intel), **release** build, and assembles a complete bundle at `dist/Donny.app` with Info.plist, entitlements, and resources. The bundle is signed **ad-hoc** by default — good enough to run on your own machine, but Gatekeeper will flag it on other Macs. For a proper signed + notarized build, see [RELEASING.md](RELEASING.md).
...
├── Bridge/DonnyBridgeFFI.h       # C header for the Rust FFI surface
├── Resources/                          # Info.plist, entitlements, AppIcon.icns
└── Sources/Donny/             # Swift UI and AppDelegate

crates/
├── donny-bridge/                # staticlib + rlib; cdylib-free
│   └── src/
...
└── donny-core/                  # Pure-Rust shared types (no I/O)
...
The C header at [apps/donny-macos/Bridge/DonnyBridgeFFI.h](../apps/donny-macos/Bridge/DonnyBridgeFFI.h) is **hand-maintained** — we don't currently auto-generate it with cbindgen. When you add or change an `extern "C"` function in `crates/donny-bridge/src/lib.rs`, update the header by hand and keep the shape in sync.
...
RUST_LOG=donny_bridge=debug ./scripts/dev.sh
```
...
2. In Xcode, **Debug → Attach to Process by PID or Name…** → `Donny`.
...
- `~/Library/Application Support/donny/settings.json`
- `~/Library/Application Support/donny/models/*.bin`
...
swift test --package-path apps/donny-macos
```
