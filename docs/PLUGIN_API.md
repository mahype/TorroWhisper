...
[`PluginHost`](../crates/donnywhisper-bridge/src/plugin_api.rs):
...
[`BridgeHost`](../crates/donnywhisper-bridge/src/plugin_api.rs). Er hält einen
...
Definiert in `crates/donnywhisper-bridge/src/plugin_api.rs`.
...
```rust
use donnywhisper_bridge::plugin_api::{BridgeHost, PluginHost, LogLevel};
...
(siehe `builtin_plugin_catalog()` in `crates/donnywhisper-core/src/lib.rs`).
...
zusammengebaut in `crates/donnywhisper-bridge/src/llm/registry.rs`. Sie enthält:
...
(`crates/donnywhisper-core/src/lib.rs`):
...
`crates/donnywhisper-bridge/src/plugin_log.rs`). So lässt sich später exakt
...
`crates/donnywhisper-bridge/src/pipeline/mod.rs`:
...
- **Konfiguration** je Schritt: `PipelineStepConfig`
  (`crates/donnywhisper-core/src/lib.rs`) trägt `stage_id`, `enabled` und ein
...
- **Chat-Plugin** — `crates/donnywhisper-bridge/src/chat.rs`,
  `ChatController::start_generation`. Baut `BridgeHost::new("chat", …)`, loggt über
  den Host und ruft `host.chat(...)` im Worker-Thread.
- **LLM-Nachbearbeitungs-Stage** —
  `crates/donnywhisper-bridge/src/pipeline/stages/llm.rs`, `LlmStage::run`. Baut
...
fn answer(settings: donnywhisper_core::AppSettings, frage: &str) -> Result<String, String> {
...
| Zweck | Datei |
|-------|-------|
| Host-Vertrag (`PluginHost`, `BridgeHost`) | `crates/donnywhisper-bridge/src/plugin_api.rs` |
| Modell-Registry-Bau | `crates/donnywhisper-bridge/src/llm/registry.rs` |
| Backend-Dispatch | `crates/donnywhisper-bridge/src/llm/mod.rs` (`provider_for`) |
| Modell-Typen / Verfügbarkeit | `crates/donnywhisper-core/src/lib.rs` (`LlmModelRef`, `LlmRegistryEntryDto`, `LlmAvailability`) |
| Logging-Fassade | `crates/donnywhisper-bridge/src/plugin_log.rs` |
| Pipeline-Stage-Verträge | `crates/donnywhisper-bridge/src/pipeline/mod.rs` |
| Plugin-Katalog (IDs, Metadaten) | `crates/donnywhisper-core/src/lib.rs` (`builtin_plugin_catalog`) |
| Referenz: Chat-Plugin | `crates/donnywhisper-bridge/src/chat.rs` |
| Referenz: LLM-Stage | `crates/donnywhisper-bridge/src/pipeline/stages/llm.rs` |
