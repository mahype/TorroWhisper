...
struct UpdatesSettingsView: View {
    ...
    var body: some View {
        ...
        Section {
            if updaterController.isAvailable {
                Text("DonnyWhisper checks for new versions at launch and then every 24 hours. Updates download in the background and install the next time you restart.", bundle: .module)
                    .font(.callout)
                    .foregroundStyle(.secondary)
            } else {
