...
final class AppModel: ObservableObject {
    ...
    private func permissionDiagnosticItems() -> [DiagnosticItemDTO] {
        ...
        if microphoneAuthorizationStatus == .authorized {
            ...
            microphone = DiagnosticItemDTO(
                ...
                recommendation: L("Enable Donny under Microphone in System Settings → Privacy & Security.", locale: locale)
            )
        }
        ...
        if accessibilityTrusted {
            ...
            accessibility = DiagnosticItemDTO(
                ...
                recommendation: L("Enable Donny under Accessibility in System Settings → Privacy & Security so it can type into other apps.", locale: locale)
            )
        }
    ...
    func clearHotkeyCapture() {
        settings.hotkey = hotkeyBeforeCapture
        hotkeyCapturePreview = ""
        hotkeyCaptureError = L("Donny needs a global hotkey. Empty input is not allowed.", locale: settings.effectiveLocale)
        isCapturingHotkey = false
    }
    ...
    var accessibilityPermissionSummary: String {
        ...
        return accessibilityTrusted
            ...
            : L("Accessibility access is missing. Enable Donny in System Settings.", locale: locale)
    }
