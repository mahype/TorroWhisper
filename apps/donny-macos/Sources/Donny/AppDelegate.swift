...
final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate, NSWindowDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        statusItem.button?.toolTip = "Donny"
    ...
    @objc private func showSettings(_ sender: Any?) {
        let window = settingsWindow ?? makeWindow(
            title: L("Donny Settings", locale: currentLocale),
    ...
    @objc private func showOnboarding(_ sender: Any?) {
        ...
        let window = onboardingWindow ?? makeWindow(
            title: L("Donny Setup", locale: currentLocale),
    ...
    private func refreshWindowTitles() {
        let locale = currentLocale
        settingsWindow?.title = L("Donny Settings", locale: locale)
        chatWindow?.title = L("Chat", locale: locale)
        onboardingWindow?.title = L("Donny Setup", locale: locale)
    ...
    private func statusImage(recording: Bool) -> NSImage? {
        ...
        let image = NSImage(systemSymbolName: symbolName, accessibilityDescription: "Donny")
    ...
    private func installEscapeCarbonHotKey() {
        ...
        if escapeEventHandler == nil {
            ...
            if installStatus != noErr {
                NSLog("Donny: failed to install Escape event handler (OSStatus %d)", Int(installStatus))
        ...
        NSLog("Donny: failed to register Escape hotkey (OSStatus %d)", Int(registerStatus))
    }
