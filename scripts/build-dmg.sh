#!/usr/bin/env bash
# Packages dist/Donny.app into a distribution DMG with a drag-to-Applications
...
create-dmg \
    --volname "Donny $VERSION" \
    --window-size 540 380 \
    --icon-size 128 \
    --icon "Donny.app" 140 190 \
    --app-drop-link 400 190 \
    --hide-extension "Donny.app" \
    --no-internet-enable \
    "$dmg" "$staging"
...
( cd dist && shasum -a 256 "Donny-$VERSION.dmg" > SHA256SUMS.txt )
