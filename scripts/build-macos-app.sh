#!/usr/bin/env bash
# Builds a universal (arm64 + x86_64) release .app bundle at dist/Donny.app.
#
...
export VERSION
echo "==> Building Donny $VERSION"

# --- Clean debug artifact so the release linker can't accidentally pick it ---
# Package.swift hard-codes `-L ../../target/debug -ldonny_bridge` for the
...
rm -f target/debug/libdonny_bridge.a
...
if $build_universal; then
    ...
    cargo build --release --target aarch64-apple-darwin -p donny-bridge -p donny-llm-helper

    echo "==> Building Rust static library + LLM helper for x86_64-apple-darwin"
    cargo build --release --target x86_64-apple-darwin -p donny-bridge -p donny-llm-helper

    echo "==> Lipo'ing universal Rust static library"
    mkdir -p target/universal/release
    lipo -create \
        target/aarch64-apple-darwin/release/libdonny_bridge.a \
        target/x86_64-apple-darwin/release/libdonny_bridge.a \
        -output target/universal/release/libdonny_bridge.a

    echo "==> Lipo'ing universal LLM helper"
    lipo -create \
        target/aarch64-apple-darwin/release/donny-llm-helper \
        target/x86_64-apple-darwin/release/donny-llm-helper \
        -output target/universal/release/donny-llm-helper
    rust_lib_dir="$repo_root/target/universal/release"
else
    echo "==> Building Rust static library + LLM helper for $native_rust_target"
    cargo build --release --target "$native_rust_target" -p donny-bridge -p donny-llm-helper
    rust_lib_dir="$repo_root/target/$native_rust_target/release"
fi
lipo -info "$rust_lib_dir/libdonny_bridge.a"
lipo -info "$rust_lib_dir/donny-llm-helper"
...
# binary built against a STALE libdonny_bridge.a (e.g. missing newly
# added settings fields). Delete the linked executables so SwiftPM must relink.
rm -f \
    "apps/donny-macos/.build/release/Donny" \
    "apps/donny-macos/.build/apple/Products/Release/Donny" \
    2>/dev/null || true
...
if $build_universal; then
    ...
    swift build \
        ...
        --package-path apps/donny-macos \
        -Xlinker -L -Xlinker "$rust_lib_dir"
    swift_build_bin="apps/donny-macos/.build/apple/Products/Release/Donny"
else
    ...
    swift build \
        ...
        --package-path apps/donny-macos \
        -Xlinker -L -Xlinker "$rust_lib_dir"
    swift_build_bin="apps/donny-macos/.build/release/Donny"
fi
...
cp "$swift_build_bin" "$app/Contents/MacOS/Donny"
cp apps/donny-macos/Resources/Info.plist "$app/Contents/Info.plist"
...
cp "$rust_lib_dir/donny-llm-helper" "$app/Contents/MacOS/donny-llm-helper"
chmod +x "$app/Contents/MacOS/donny-llm-helper"

if [[ -f apps/donny-macos/Resources/AppIcon.icns ]]; then
    cp apps/donny-macos/Resources/AppIcon.icns "$app/Contents/Resources/AppIcon.icns"
fi

# --- Copy in-app localization tables (Bundle.module) --------------------------
# The Donny target does NOT declare `resources:` in Package.swift —
...
for lproj_dir in apps/donny-macos/Sources/Donny/Resources/*.lproj; do
...
if ! $copied_localizable; then
    echo "error: no Localizable.strings found under Sources/Donny/Resources/*.lproj" >&2
    exit 1
fi
...
for lproj_dir in apps/donny-macos/Resources/Localizations/*.lproj; do
    if [[ -d "$lproj_dir" ]]; then
        lang_name="$(basename "$lproj_dir")"
        mkdir -p "$app/Contents/Resources/$lang_name"
...
sparkle_framework_src="apps/donny-macos/.build/artifacts/sparkle/Sparkle/Sparkle.xcframework/macos-arm64_x86_64/Sparkle.framework"
if [[ ! -d "$sparkle_framework_src" ]]; then
    echo "error: Sparkle.framework not found at $sparkle_framework_src" >&2
    echo "       run 'swift package --package-path apps/donny-macos resolve' first" >&2
    exit 1
fi
...
install_name_tool -add_rpath "@executable_path/../Frameworks" "$app/Contents/MacOS/Donny"
...
entitlements="apps/donny-macos/Resources/Donny.entitlements"
...
if [[ -n "${MACOS_SIGN_IDENTITY:-}" ]]; then
    ...
    codesign --force --timestamp --options=runtime \
        ...
        "$app/Contents/MacOS/donny-llm-helper"
    ...
    codesign --force --sign - "$app/Contents/MacOS/donny-llm-helper"
